...
async def reportStuff(self, ctx, message):
	...
	embed = discord.Embed(title=f"Report from {ctx.message.author}",
...
def save_leaderboard():
	...
	screenshot = driver.find_element_by_id('table').screenshot(
		'leaderboard.png')
	driver.quit()
	#transparency time
...
class Utils(commands.Cog):
	...
	async def findseed(self, ctx):
		"""Test your luck"""
		if ctx.message.channel.id != int(self.bot.config[str(
				ctx.message.guild.id)]["bot_channel"]):
	...
	async def someone(self, ctx):
		"""Discord's mistake"""
		if ctx.channel.id != int(self.bot.config[str(
				ctx.message.guild.id)]["fair_channel"]):
